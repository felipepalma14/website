(function() {
	...
	angular
		...
		TesteCtrl.$inject = ['$firebaseArray','$location',
							'AuthenticationService','$rootScope',
							'APIService','FIPEService','$scope'
							];

		function TesteCtrl($firebaseArray,$location,AuthenticationService,$rootScope,APIService,FIPEService,$scope){
			$scope.marcas  =  [];	
			$scope.modelos =  [];
			$scope.anos    =  [];
			...
			*/
			$scope.addProdutoToFirebase2 = function (produto){
				if($scope.produtoType['$id']){
					...
					APIService.addProduto2(produto,function(resultKeyProduto){
